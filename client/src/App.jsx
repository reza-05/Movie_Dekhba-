...
function App() {
  ...
  const [selectedCategory, setSelectedCategory] = useState('movies'); // 'movies', 'anime', 'series'
  const [isSyncingCatalog, setIsSyncingCatalog] = useState(false);
  const [syncStatus, setSyncStatus] = useState('');

  const handleSyncCatalog = async () => {
    setIsSyncingCatalog(true);
    setSyncStatus('Syncing Telegram...');
    try {
      const backendUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:5001';
      const res = await fetch(`${backendUrl}/api/sync-telegram`, { method: 'POST' });
      if (res.ok) {
        const data = await res.json();
        setSyncStatus(`Synced! +${data.addedCount} items`);
        // Reload catalog
        const catRes = await fetch(`${backendUrl}/api/movies-catalog`);
        if (catRes.ok) {
          const catData = await catRes.json();
          setMoviesCatalog(catData);
        }
      } else {
      ...
      <main className="flex-grow flex items-center justify-center px-4 sm:px-8 py-6 sm:py-12 md:py-16 lg:py-20 relative z-10">
        {activeView === 'landing' ? (
          ...
          <div className="w-full max-w-6xl px-4 sm:px-6 lg:px-8 flex flex-col gap-8 text-left animate-fade-in select-none">
            {/* Category Navigation Tabs & Sync Button */}
            <div className="flex flex-col gap-4 border-b border-white/[0.04] pb-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h2 className="text-2xl md:text-3xl font-black text-white tracking-tight">Cinematic Catalog</h2>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:scale-[0.97] disabled:opacity-50 text-white rounded-xl text-xs font-bold transition-all shadow-md flex items-center gap-1.5 cursor-pointer"
                >
                  <Sparkles className={`h-3.5 w-3.5 ${isSyncingCatalog ? 'animate-spin' : ''}`} />
                  {syncStatus || 'Sync Telegram'}
                </button>
              </div>

              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-2">
                {/* Category Navigation Pills */}
                <div className="flex items-center gap-2 bg-slate-950/40 p-1.5 rounded-xl border border-white/[0.06] backdrop-blur-md self-start">
                  {[
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSelectedGenre('All'); // Reset genre filter when switching categories
                      }}
            ...
            {moviesCatalog.filter(movie => {
              const matchesCategory = movie.category === selectedCategory || (!movie.category && selectedCategory === 'movies');
              ...
              <div className="py-16 text-center text-slate-500">
                ...
                <p className="text-xs mt-1">Try refining your search or select another category/genre.</p>
              </div>
            ) : (
